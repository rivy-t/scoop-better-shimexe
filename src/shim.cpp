...
std::tuple<std::wstring_p, std::wstring_p> GetShimInfo() {
  ...
  while (true) {
    ...
    if (line.substr(0, 4) == L"path") {
      ...
      if (line_substr.find(L" ") != std::wstring_view::npos &&
          line_substr.front() != L'"') {
        path.emplace(L"\"");
        auto &path_value = path.value();
        path_value.append(line_substr.data(),
                          line_substr.size() - (line.back() == L'\n' ? 1 : 0));
        path_value.push_back(L'"');
      } else {
        path.emplace(line_substr.data(),
                     line_substr.size() - (line.back() == L'\n' ? 1 : 0));
      }
    ...
    if (line.substr(0, 4) == L"args") {
      args.emplace(line.data() + 7,
                   line.size() - 7 - (line.back() == L'\n' ? 1 : 0));
      continue;
    }
  }
...
std::tuple<std::unique_handle, std::unique_handle>
MakeProcess(const std::wstring_p &path, const std::wstring_p &args) {
  // Start subprocess
  STARTUPINFOW si = {};
  ...
  if (CreateProcessW(nullptr, cmd.data(), nullptr, nullptr, TRUE,
    ...
    if (GetLastError() == ERROR_ELEVATION_REQUIRED) {
      ...
      // which CAN create elevated processes, at the cost of opening a new
      // separate window. Theorically, this could be fixed (or rather, worked
      // around) using pipes and IPC, but... this is a question for another day.
      ...
      if (!ShellExecuteExW(&sei)) {
        fprintf(stderr, "Unable to create elevated process: error %li.",
                GetLastError());
  ...
  if (!SetConsoleCtrlHandler(CtrlHandler, TRUE)) {
    fprintf(stderr,
            "Could not set control handler; Ctrl-C behavior may be invalid.\n");
  }
...
int main(int _argc, char *_argv[]) {
  ...
  SHFILEINFOW sfi = {};
  const auto ret =
      SHGetFileInfoW(unquotedPath, -1, &sfi, sizeof(sfi), SHGFI_EXETYPE);

  if (ret == 0) {
    fprintf(stderr,
            "Could not determine if target is a GUI app. Assuming console.\n");
  }
  ...
  auto [processHandle, threadHandle] =
      MakeProcess(std::move(path), std::move(args));
